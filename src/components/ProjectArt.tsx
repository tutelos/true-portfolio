 ...
 */
export default function ProjectArt({ art, name }: { art: Project['art']; name: string }) {
  return (
    <div
      role="img"
    ...
    >
      ...
      {art === 'atlas' && (
        <>
          <span className="display absolute -bottom-[18%] -right-[2%] select-none text-[38vw] leading-none text-white/[0.045] md:text-[16vw]">
            A
          </span>
          <div className="absolute left-[10%] top-[18%] h-24 w-24 border border-white/15 md:h-36 md:w-36">
      ...
      {art === 'atlas-landing' && (
        <>
          <div className="absolute left-[12%] right-[12%] top-[22%] space-y-4">
            ...
            <div className="mt-8 h-9 w-28 border border-white/20" />
          </div>
          <span className="display absolute -bottom-[14%] left-[8%] select-none text-[30vw] leading-none text-white/[0.04] md:text-[12vw]">
            LP
          </span>
        </>
      )}
      ...
      {art === 'rubiks' && (
        <div className="absolute left-1/2 top-1/2 grid -translate-x-1/2 -translate-y-1/2 grid-cols-3 gap-1.5">
          {Array.from({ length: 9 }).map((_, i) => (
            <div
              key={i}
              className="h-10 w-10 border border-white/15 md:h-14 md:w-14"
          ...
          {[10, 9, 8, 4].map((keys, row) => (
            <div key={row} className="flex gap-1.5">
              {Array.from({ length: keys }).map((_, i) => (
                <div
                  key={i}
                  className={`h-8 border border-white/15 md:h-10 ${
      ...
      {art === 'tws' && (
        <>
          <div className="absolute left-[32%] top-1/2 h-20 w-20 -translate-y-1/2 rounded-full border border-white/20 md:h-28 md:w-28">
            <div className="absolute inset-4 rounded-full border border-white/10" />
          </div>
          <div className="absolute right-[32%] top-1/2 h-20 w-20 -translate-y-[40%] rounded-full border border-white/20 md:h-28 md:w-28">
            <div className="absolute inset-4 rounded-full border border-white/10" />
          </div>
      ...
      <span className="label-xs absolute bottom-3 right-4">1920 × 1080</span>
    </div>
  )
}
